...
const FrameSelect = () => {
    ...
    const resetCustomFramework = () => {
        ...
        setCustomFramework((prevState) => {
            for (let category of top_categories) {
                ...
                if (category === "environmental_risk_metrics" || category === "social_risk_metrics" || category === "governance_risk_metrics") {
                    for (let metric in CustomMetricOppOrRisk[category]) {
                        CustomMetricOppOrRisk[category][metric] = false;
                    }
                } else if (category === "environmental_opportunity_metrics" || category === "social_opportunity_metrics" || category === "governance_opportunity_metrics") {
                    for (let metric in CustomMetricOppOrRisk[category]) {
  ...
  const handleDelete = async (user_name, frameworkName) => {
    ...
    const handleToEditWindow = () => {
        ...
        setCustomFramework((prevState) => {
            for (let category of top_categories) {
                let matchedKey = Object.keys(currentFramework[category]).find(key => regex.test(key));
                prevState[category].indicator_weight = currentFramework[category][matchedKey];
                ...
                for (let metric in currentFramework[category]) {
                    if (metric in prevState[category].metrics) {
                        prevState[category].metrics[metric] = currentFramework[category][metric];
          ...
          <div className="col-3 mb-4">
            ...
            >
              <div className="text-center">
                <img src={`FolderAdd.png`} alt="Create"
              ...
              <div key={index} className="col-3 mb-4">
                <div className="card h-100 d-flex flex-column justify-content-between shadow card-hover">
                  ...
                  <div className="card-footer bg-transparent border-0 d-flex justify-content-center"
                    ...
                    {!["IFRS S1", "IFRS S2", "TCFD", "TNFD", "APRA-CPG"].includes(framework.framework_name) && (
                        <img
                            src="Delete.png"
                            alt="Delete"
                            style={{
