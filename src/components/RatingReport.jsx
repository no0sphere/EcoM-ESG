...
const RatingReport = ({ data, pieData, pieOptions, barData, barOptions, descriptions, Industry, Company, Year }) => {
    ...
    return (
        <div className="container mt-4" style={{
            ...
            <h3 className="text-center" style={{ fontWeight: 600, color: '#333' }}>{Industry.label} - {Company} ({Year})</h3>
            <p style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
                Company Rating: {data[0]['Rating of Company']}
                <img src={getComparisonImage(parseFloat(data[0]['Rating of Company']), parseFloat(data[1]['Average Rating of Industry']))} alt="comparison" style={{ marginLeft: '5px', width: '24px', height: '24px' }} />
            </p>
            <p style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Rating Average: {data[1]['Average Rating of Industry']}</p>
            <p style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Most valuable weight: {maxWeight.label} - {convertToPercentage(maxWeight.value)}%</p>
            <div className="row">
            ...
            <div className="mt-4" style={{ textAlign: 'left' }}>
                {Object.keys(data[0]).map((key, index) => (
                    ...
                    <p key={index} style={{ fontSize: '1.1rem' }}>
                        <strong>{weightNames[key] || key}:</strong> The company's value is &nbsp;
                        <span style={{ color: getTextColor(parseFloat(data[0][key]), parseFloat(data[1][key])) }}>
                            {convertToPercentage(data[0][key])}%
                        </span>
                        <img src={getComparisonImage(parseFloat(data[0][key]), parseFloat(data[1][key]))} alt="comparison" style={{ marginLeft: '5px', width: '16px', height: '16px' }} />
                        , compared to the industry average of &nbsp;
                        {convertToPercentage(data[1][key])}%
                        . {descriptions[index]}
                    </p>
                ))}
                <p style={{ fontSize: '1.1rem' }}><strong>Average Rating of Industry:</strong> The industry average rating
                    is {data[1]['Average Rating of Industry']}.
                </p>
            </div>
        </div>
    );
